...
def test_init_scaffolding(runner, temp_project_dir, mock_templates, monkeypatch):
    ...
    monkeypatch.setattr("skillsmith.cli.TEMPLATE_DIR", mock_templates)
    monkeypatch.setattr("skillsmith.cli.PLATFORM_DIR", mock_templates / "platforms")
    
    with runner.isolated_filesystem(temp_dir=temp_project_dir):
        result = runner.invoke(main, ["init"])
        
        assert result.exit_code == 0
        assert "Successfully initialized .agent structure!" in result.output
        
        assert os.path.exists("AGENTS.md")
        assert os.path.exists(".agent/prd.md")
        assert os.path.exists(".agent/status.md")
        assert os.path.exists(".agent/skills/test-skill/SKILL.md")
...
def test_lint_failure(runner, temp_project_dir, mock_templates, monkeypatch):
    ...
    # Create an invalid skill in the current directory's .agent/skills
    with runner.isolated_filesystem(temp_dir=temp_project_dir):
        skills_dir = Path(".agent/skills/bad-skill")
...
def test_update_skill(runner, temp_project_dir, mock_templates, monkeypatch):
    ...
    with runner.isolated_filesystem(temp_dir=temp_project_dir):
        ...
        tpl_skill_md = mock_templates / ".agent" / "skills" / "test-skill" / "SKILL.md"
        ...
        local_md = Path(".agent/skills/test-skill/SKILL.md")
